...
const table = state.players
  .map((p, i) => {
    ...
    return `${place} *${p.nickname}* — ${money(p.dollars)} · ${p.tier.name} · ${trend} (7d)${streak}`;
  })
  .join('\n');
...
const moverLine =
  mover && mover.move !== 0
    ? `${mover.move > 0 ? '\u{1F4C8}' : '\u{1F4C9}'} *${mover.player.nickname}* ${signed(mover.move)} in the last day`
...
const blocks = [
  ...
  {
    type: 'context',
    elements: [{ type: 'mrkdwn', text: `\u{1F3E6} *${money(state.bank)}* still unclaimed in the bank · ${state.daysLeft} days left in ${data.season.name}` }],
  },
  { type: 'section', text: { type: 'mrkdwn', text: `*Biggest mover:* ${moverLine}` } },
  { type: 'section', text: { type: 'mrkdwn', text: `\u{1F3AF} *${ch.title}:* ${ch.desc}` } },
